...
function embaralharQuestoes(questoes) {
  return questoes.sort(() => Math.random() - 0.5); // embaralha as perguntas do array questoes
}

// Função para exibir uma pergunta e obter a resposta do usuário
function exibirPergunta(perguntaSelecionada) {
...
function validarRespostaDoUsuario(respostaDoUsuario, perguntaSelecionada) {
  return respostaDoUsuario === perguntaSelecionada.resposta;
}

// Função para exibir a mensagem final com base na pontuação
...
function iniciarQuiz() {
  ...
  for (let i = 0; i < quantidadeQuestoes; i++) {
    const perguntaSelecionada = questoes[i];
    const respostaUsuario = exibirPergunta(perguntaSelecionada);
    const resultadoValidacao = validarRespostaDoUsuario(respostaUsuario, perguntaSelecionada);

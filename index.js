...
function selecionarQuestoesAleatorias(questoes, quantidadeQuestoes) {
  ...
  const questoesSelecionadas = questoes.sort(() => Math.random() - 0.5)

  return questoesSelecionadas.slice(0, quantidadeQuestoes);
}

// Função responsável por exbir a pergunta e retornar a resposta do usuário
...
// Função responsável por validar a resposta do usuário
function validarRespostaDoUsuario(respostaDoUsuario, pergunta) {
  return respostaDoUsuario === pergunta.resposta;
}


// Função responsável por retornar uma mensagem com base na pontuação do jogador
...
function iniciarQuiz() {
  ...
  // Variável que armazena as 10 perguntas aleatórias
...
console.log(Math.random() - 0.5); // Resultado = 0.21511260726177595
